    ...
    <header class="entry-header">
		...
		<div class="entry-date">
			<?php if($mtime = get_mtime('Y.m.d')) { ?>
				公開投稿日：
				<span itemprop="datePublished" content="<?php echo get_post_time('Y-m-d') ?>" class="published">
				<?php echo get_post_time('Y.m.d') ?>
				</span>
				最終更新日：
				<time itemprop="dateModified" datetime="<?php echo get_mtime('Y-m-d') ?>" class="updated">
					<?php echo $mtime ?>
				</time>
			<?php } else { ?>
				公開日：
				<time itemprop="datePublished" datetime="<?php echo get_post_time('Y-m-d') ?>" pubdate="pubdate" class="published updated">
					<?php echo get_post_time('Y.m.d') ?>
				</time>
			<?php } ?>
		</div>	
